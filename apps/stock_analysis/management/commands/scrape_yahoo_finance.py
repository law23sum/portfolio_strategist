...
from apps.stock_analysis.tasks import scrape_yahoo_finance_comprehensive, refresh_all_stock_snapshots_comprehensive
        ...
        if async_task:
            ...
            for symbol in symbols:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f'Queued task for {symbol} (task ID: {task.id})')
                    )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f'Failed to queue task for {symbol}: {e}')
                    )
        else:
            # Run synchronously
            successful = 0
            ...
            for symbol in symbols:
                try:
                    ...
                    if result.get('success'):
                        ...
                        self.stdout.write(
                            self.style.ERROR(f'✗ {symbol}: Failed - {error}')
                        )
                except Exception as e:
                    failed += 1
                    self.stdout.write(
                        self.style.ERROR(f'✗ {symbol}: Exception - {str(e)}')
                    )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'Completed: {successful} successful, {failed} failed out of {len(symbols)} total'
                )
            )
