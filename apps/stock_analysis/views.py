...
from .forms import InvestmentPlanForm, PersonalLoanForm, StockAnalysisForm  # noqa: E402
from .models import InvestmentPlan, PersonalLoanAnalysis, StockAnalysis  # noqa: E402
from .services import StockAnalysisService  # noqa: E402


def _json_safe(value):
    """
    Convert numpy/pandas scalar types into JSON-serializable primitives.

    The stock analysis pipeline produces numpy floats/ints and pandas timestamps
    that need normalising before they can be stored in JSONFields.
    """
...
def analyze_stock(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if result:
                stock_data = _json_safe(result.get("stock_data") or {})
                ratios_table = _json_safe(result.get("ratios_table") or [])
                ai_assessment = result.get("ai_assessment", "")
                forecast_data = _json_safe(result.get("forecast_data") or [])
                news_html = result.get("news_html", "")
                history_data = _json_safe(result.get("history_data") or [])
                forecast_errors = _json_safe(result.get("forecast_errors") or [])
                risk_metrics = _json_safe(result.get("risk_metrics") or {})
                risk_insights = _json_safe(result.get("risk_insights") or [])
                market_overview = _json_safe(result.get("market_overview") or {})
                decision_support = _json_safe(result.get("decision_support") or {})
                benchmark_symbol = result.get("benchmark_symbol") or "^GSPC"
...
def analysis_results(request, pk):
    ...
    stock_data = analysis.stock_data if isinstance(analysis.stock_data, dict) else {}
    stock_data_items = []
    for key, value in stock_data.items():
        stock_data_items.append((key, value))
    stock_data_items.sort(key=lambda item: item[0])

    ratios_df = _to_dataframe(analysis.ratios_table)

    raw_forecast = analysis.forecast_data
    forecast_records = []
    forecast_summary = []
    forecast_statistics = {}
    if isinstance(raw_forecast, dict):
        forecast_records = raw_forecast.get("forecast_data", [])
        forecast_summary = raw_forecast.get("summary", [])
        forecast_statistics = raw_forecast.get("statistics", {})
    elif raw_forecast:
        forecast_records = raw_forecast
    forecast_df = _to_dataframe(forecast_records)
    if not forecast_df.empty and "Date" in forecast_df.columns:
        forecast_df["Date"] = pd.to_datetime(forecast_df["Date"], errors="coerce")

    forecast_errors_df = _to_dataframe(analysis.forecast_errors)
    if not forecast_errors_df.empty and "date" in forecast_errors_df.columns:
        forecast_errors_df["date"] = pd.to_datetime(forecast_errors_df["date"], errors="coerce")

    risk_metrics = _json_safe(analysis.risk_metrics or {})
    risk_insights = analysis.risk_insights if isinstance(analysis.risk_insights, list) else []

    risk_metric_config = [
        ("annual_return", "Annual Return", "pct"),
        ("annual_volatility", "Annual Volatility", "pct"),
        ("sharpe_ratio", "Sharpe Ratio", "raw"),
        ("sortino_ratio", "Sortino Ratio", "raw"),
        ("max_drawdown", "Max Drawdown", "pct"),
        ("value_at_risk_95", "Value at Risk (95%)", "pct"),
        ("beta", "Beta", "raw"),
        ("benchmark_annual_return", f"{analysis.benchmark_symbol} Annual Return", "pct"),
        ("relative_return", f"Relative Return vs {analysis.benchmark_symbol}", "pct"),
    ]

    risk_metric_rows = []
    for key, label, fmt in risk_metric_config:
        value = risk_metrics.get(key)
        safe_value = _json_safe(value)
        risk_metric_rows.append(
            {
                "label": label,
                "value": safe_value,
                "format": fmt,
    ...
    market_overview = _json_safe(analysis.market_overview or {})
    decision_support = _json_safe(analysis.decision_support or {})

    return render(
        request,
        "stock_analysis/results.html",
        {
            "analysis": analysis,
            "ratios_df": ratios_df,
            ...
            "forecast_errors_df": forecast_errors_df,
            "stock_data_items": stock_data_items,
            "market_overview": market_overview,
            "risk_metrics": risk_metrics,
...
def download_pdf(request, pk):
    ...
    service.generate_pdf_report(
        analysis.symbol, analysis.stock_data, analysis.ratios_table, analysis.ai_assessment, str(output_path)
    )
...
def personal_loan_analysis(request):
    ...
    return render(
        request,
        "stock_analysis/personal_loan.html",
        {
            "form": form,
            "analyses": analyses,
            "active_tab": "stock_analysis",
...
def loan_results(request, pk):
    ...
    return render(
        request,
        "stock_analysis/loan_results.html",
        {
            "analysis": analysis,
...
def investment_forecast_api(request):
    ...
    try:
            ...
            if not merged.empty:
                ...
                if min_len > 10:
                    ...
                    external_factors = {
                        ...
                        "factor_betas": {k: float(v) if np.isfinite(v) else 0.0 for k, v in factor_betas.items()}
        ...
        if forecast_df is None or forecast_df.empty:
            ...
            forecast_prices = current_price * np.exp((mu_daily - 0.5 * sigma_daily**2) * t_forecast + sigma_daily * W)
            forecast_df = pd.DataFrame(
                {
                    "Date": [
                        pd.to_datetime(history_df["Date"].iloc[-1]) + pd.Timedelta(days=int(t)) for t in t_forecast
                    ],
            ...
            formatted_summary.append(
                {
                    ...
                    "investment_value": float(entry["investment_value"])
                    if entry["investment_value"] is not None
                    else None,
...
def get_stock_analysis_forecast(request):
    ...
    try:
        ...
        analysis = StockAnalysis.objects.filter(user=request.user, symbol=symbol).order_by("-analysis_date").first()
...
def stock_details_api(request):
    ...
    try:
        ...
        # Initialize service early as it may be needed in multiple code paths
        service = StockAnalysisService()
        ...
        try:
            ...
            logger.info(
                f"Running synchronous Yahoo Finance web scraping for {symbol} to get fresh current price (force_refresh={force_refresh})"
            )
            ...
            try:
                ...
                logger.info(
                    f"Triggered async comprehensive Yahoo Finance web scraping task for {symbol} (force_refresh={force_refresh})"
                )
        ...
        for item in news_items:
            ...
            if item.get("summary") and isinstance(item.get("summary"), dict):
                ...
                publisher = (
                    summary.get("provider", {}).get("displayName")
                    or summary.get("provider", {}).get("name")
                    or item.get("publisher", item.get("source", ""))
                )
