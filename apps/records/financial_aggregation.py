...
class BudgetAggregationService:
    ...
    def get_user_budget_data(user: User, days: int = 30) -> Dict[str, Any]:
        ...
        accounts = LinkedAccount.objects.filter(user=user, status="active", account_type__in=["depository", "credit"])
        ...
        category_spending = [
            ...
            for cat, data in sorted(spending_by_category.items(), key=lambda x: x[1]["amount"], reverse=True)[:10]
        ]
...
class InvestmentAggregationService:
    ...
    def get_user_investment_data(user: User) -> Dict[str, Any]:
        ...
        holdings = (
            ...
            .select_related("account")
            .order_by("-as_of_date", "security_name")
        )
        ...
        holdings_by_security = defaultdict(
            lambda: {
        ...
        # Convert to list
        security_list = []
        for security_key, data in sorted(holdings_by_security.items(), key=lambda x: x[1]["value"], reverse=True):
...
class DebtAggregationService:
    ...
    def get_user_debt_data(user: User) -> Dict[str, Any]:
        ...
        debt_accounts = LinkedAccount.objects.filter(user=user, status="active", account_type__in=["credit", "loan"])
        ...
        debt_type_list = [
            ...
            for debt_type, data in sorted(debt_by_type.items(), key=lambda x: x[1]["balance"], reverse=True)
        ]
...
class DashboardAggregationService:
    ...
    def get_user_financial_summary(user: User) -> Dict[str, Any]:
        ...
        all_accounts = LinkedAccount.objects.filter(user=user, status="active")
        ...
        for account in all_accounts:
            ...
            if latest_balance:
                ...
                if account.account_type in ["depository"]:
                    ...
                    holdings_value = InvestmentHolding.objects.filter(account=account).aggregate(total=Sum("value"))[
                        "total"
                    ] or Decimal("0.00")
        ...
        account_counts = all_accounts.values("account_type").annotate(count=Count("id"))
