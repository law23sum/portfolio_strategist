...
try:
    ...
    logger.warning(
        f"Plaid Python SDK not installed or import error: {e}. Financial aggregation features will be limited."
    )
...
def categorize_and_connect_account(linked_account: LinkedAccount) -> Dict[str, Any]:
    ...
    """
    category_info = {"category": None, "connected": False, "details": {}}
...
class PlaidAggregationService:
    ...
    def __init__(self, provider: Optional[AggregationProvider] = None):
        ...
        if not PLAID_AVAILABLE:
            raise ImportError("Plaid Python SDK is not installed. Install it with: pip install plaid-python")
        ...
        plaid_environment = environment_map.get(self.provider.environment.lower(), "https://sandbox.plaid.com")
    ...
    def create_link_token(self, user_id: str = None, user_email: str = None, for_auth: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if user_id:
                request_data["user"] = LinkTokenCreateRequestUser(client_user_id=str(user_id))
            elif for_auth:
                # For authentication, use a temporary client_user_id
                request_data["user"] = LinkTokenCreateRequestUser(client_user_id="temp_auth_" + str(uuid.uuid4()))
    ...
    def get_identity(self, access_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            identity_data = {"accounts": [], "names": [], "emails": [], "phone_numbers": [], "addresses": []}
            ...
            for account in accounts:
                ...
                for owner in owners:
                    owner_data = {"names": [], "emails": [], "phone_numbers": [], "addresses": []}
    ...
    def sync_accounts(self, linked_account: LinkedAccount) -> DataSyncLog:
        ...
        try:
            ...
            self._sync_investment_transactions(linked_account, access_token)
        ...
        for account_data in accounts_data:
            ...
            AccountBalance.objects.update_or_create(
                ...
                defaults={
                    ...
                    "available_balance": Decimal(str(account_data.get("balances", {}).get("available", 0)))
                    if account_data.get("balances", {}).get("available") is not None
                    else None,
                    "limit": Decimal(str(account_data.get("balances", {}).get("limit", 0)))
                    if account_data.get("balances", {}).get("limit") is not None
                    else None,
    ...
    def _sync_transactions(self, linked_account: LinkedAccount, access_token: str, days_back: int = 90) -> int:
        ...
        try:
            ...
            for tx_data in transactions:
                ...
                category_str = (
                    ", ".join(categories) if isinstance(categories, list) else str(categories) if categories else ""
                )
    ...
    def _sync_investment_transactions(
        self, linked_account: LinkedAccount, access_token: str, days_back: int = 90
    ) -> int:
        ...
        try:
            ...
            transactions = (
                ...
                else response.get("investment_transactions", [])
            )
