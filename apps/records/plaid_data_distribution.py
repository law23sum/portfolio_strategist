    ...
    def get_organized_plaid_data(user, use_api_data: bool = True) -> Dict[str, Any]:
        ...
        if not provider:
            ...
            if not provider:
                ...
                logger.info(f"Attempting to find accounts without provider filter...")
            else:
        ...
        if not accounts_data or not use_api_data:
            ...
            if not linked_accounts or not linked_accounts.exists():
                if provider:
                    logger.warning(f"No linked accounts found for user {user.id} with provider {provider.name} (id: {provider.id})")
                else:
                ...
                if all_accounts.exists():
                    logger.info(f"Found {all_accounts.count()} accounts for user {user.id} without provider filter, using those")
                    ...
                    if any_accounts.exists():
                        ...
                        logger.warning(f"Found {any_accounts.count()} accounts but none with 'active' or 'pending' status.")
                        ...
                        for acc in any_accounts[:3]:
                            logger.warning(f"  Account: {acc.account_name} (status: {acc.status}, provider: {acc.provider.name if acc.provider else 'None'})")
            ...
            for account in linked_accounts:
                ...
                balance = float(latest_balance.current_balance) if latest_balance and latest_balance.current_balance else 0.0
        ...
        def parse_account_name(account_name):
            ...
            if ('bond' in name_lower or 
                'treasury' in name_lower or 
                'municipal' in name_lower or
                'corporate bond' in name_lower or
                'government bond' in name_lower or
                'fixed income' in name_lower):
        ...
        for account_data in accounts_data:
            ...
            logger.info(f"Processing account: name='{account_name}', type='{account_type}', subtype='{subtype_str}', balance={balance}, parsed={name_parsed}")
            ...
            is_checking = name_parsed.get('is_checking') or (account_type == 'depository' and ('checking' in subtype_str or ('depository' in subtype_str and 'savings' not in subtype_str and 'cd' not in subtype_str)))
            is_savings = name_parsed.get('is_savings') or (account_type == 'depository' and 'savings' in subtype_str and 'cd' not in subtype_str)
            is_cd = name_parsed.get('is_cd') or (account_type == 'depository' and ('cd' in subtype_str or 'certificate' in subtype_str))
            ...
            is_bond = (name_parsed.get('is_bond') or 
                      'bond' in subtype_str or 
                      'bond' in account_name_lower or
                      'treasury' in account_name_lower or
                      'municipal' in account_name_lower or
                      'corporate bond' in account_name_lower or
                      'government bond' in account_name_lower or
                      'fixed income' in account_name_lower)
            
            # Investment identification - check type first, then name
            is_investment = (account_type in ['investment', 'brokerage'] or 
                           name_parsed.get('is_investment') or
                           'investment' in account_name_lower or
                           'brokerage' in account_name_lower or
                           'trading' in account_name_lower)
            
            is_retirement = (name_parsed.get('is_retirement') or 
                           account_type == 'retirement' or 
                           (account_type == 'investment' and ('401k' in subtype_str or 'ira' in subtype_str or 'retirement' in subtype_str or 'roth' in subtype_str)) or
                           '401k' in account_name_lower or
                           'ira' in account_name_lower or
                           'roth' in account_name_lower or
                           'pension' in account_name_lower)
            
            logger.info(f"  Categorization: is_bond={is_bond}, is_investment={is_investment}, is_retirement={is_retirement}, is_savings={is_savings}, is_cd={is_cd}")
            ...
            elif account_type in ['investment', 'brokerage'] and not is_bond and not is_retirement and not is_investment:
                logger.warning(f"Investment account {account_name} not properly categorized - adding to stocks as fallback")
                organized_data['stocks_assessment']['investment_amount'] += balance
                organized_data['stocks_assessment']['accounts'].append({
                    'id': str(account_id_for_list) if account_id_for_list else '',
                    'name': str(account_name) if account_name else '',
            ...
            is_retirement = (account_type == 'retirement' or 
                           name_parsed.get('is_retirement') or
                           (account_type == 'investment' and ('401k' in subtype_str or 'ira' in subtype_str or 'retirement' in subtype_str or 'roth' in subtype_str)))
        ...
        if organized_data['credit_score']['total_credit_limit'] > 0:
            organized_data['credit_score']['credit_utilization_percent'] = (
                ...
                organized_data['credit_score']['total_credit_limit'] * 100
            )
