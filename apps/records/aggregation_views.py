...
from .aggregation_service import PlaidAggregationService, categorize_and_connect_account
...
def _enqueue_sync_linked_account(linked_account_id: int) -> None:
    """Import Celery task lazily to avoid issues during module import."""
    from .tasks import sync_linked_account

    sync_linked_account.delay(linked_account_id)
...
def link_account_view(request):
    ...
    plaid_provider, _ = AggregationProvider.objects.get_or_create(
        name="plaid", defaults={"display_name": "Plaid", "is_active": True, "environment": "sandbox"}
    )
    ...
    if not plaid_provider.api_key or not plaid_provider.api_secret:
        ...
        messages.warning(
            request,
            "Plaid provider is configured but missing credentials. Please run: python manage.py bootstrap_plaid",
        )
...
def create_link_token(request):
    ...
    try:
        ...
        if not provider_id:
            return JsonResponse(
                {"error": "No provider specified. Please run: python manage.py bootstrap_plaid"}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Plaid provider not configured. Please run: python manage.py bootstrap_plaid"}, status=404
            )
        ...
        if not provider.api_key or not provider.api_secret:
            return JsonResponse(
                {"error": "Plaid credentials not configured. Please run: python manage.py bootstrap_plaid"}, status=400
            )

        service = PlaidAggregationService(provider)
        link_token_data = service.create_link_token(user_id=str(request.user.id), user_email=request.user.email)
        ...
        return JsonResponse(
            {"error": f"Plaid not configured: {str(e)}. Please run: python manage.py bootstrap_plaid"}, status=500
        )
    except ImportError as e:
        logger.error(f"Error creating link token: {e}")
        return JsonResponse(
            {"error": "Plaid Python SDK not installed. Install with: pip install plaid-python"}, status=500
        )
...
def exchange_token(request):
    ...
    try:
        ...
        created_accounts = []
        categorized_accounts = {"budget": [], "debt": [], "investment": [], "retirement": [], "other": []}
        ...
        for account_data in accounts_data:
            ...
            if subtype_raw:
                if hasattr(subtype_raw, "value"):
                    ...
                    account_subtype_lower = (
                        str(subtype_raw.value).lower() if hasattr(subtype_raw, "value") else str(subtype_raw).lower()
                    )
                    account_subtype_stored = (
                        str(subtype_raw.value) if hasattr(subtype_raw, "value") else str(subtype_raw)
                    )
                else:
            ...
            if created:
                ...
                _enqueue_sync_linked_account(linked_account.id)
...
def linked_accounts_view(request):
    ...
    linked_accounts = (
        LinkedAccount.objects.filter(user=request.user)
    ...
    return render(
        request,
        "records/linked_accounts.html",
        {
            "accounts_with_balances": accounts_with_balances,
            "active_tab": "records_linked_accounts",
        },
    )
...
def account_detail_view(request, account_id):
    ...
    holdings = (
        linked_account.holdings.all()
        if linked_account.account_type in ["investment", "brokerage", "retirement"]
        else []
    )
        ...
        account_balances = [
            {
                ...
                "account_name": getattr(item.get("account"), "account_name", ""),
                "account_type": getattr(item.get("account"), "account_type", ""),
                "institution_name": getattr(item.get("account"), "institution_name", ""),
                "balance": str(getattr(item.get("balance"), "current_balance", "0.00")),
                "available_balance": str(getattr(item.get("balance"), "available_balance", ""))
                if getattr(item.get("balance"), "available_balance", None) is not None
                else None,
                "as_of": str(getattr(item.get("balance"), "balance_date", "")),
            }
        ...
        return JsonResponse(
            {
                ...
                "net_flow": str(budget_data.get("net_flow", 0)),
                "period": {
                    "start": str(budget_data.get("start_date")) if budget_data.get("start_date") else None,
                    "end": str(budget_data.get("end_date")) if budget_data.get("end_date") else None,
                    "days": budget_data.get("period_days"),
                },
...
def investment_data_api(request):
    ...
    try:
        ...
        holdings = [
            {
                ...
                "security_ticker": holding.get("security_ticker"),
                "security_type": holding.get("security_type"),
                "quantity": str(holding.get("quantity", 0)),
                "value": str(holding.get("value", 0)),
                "cost_basis": str(holding.get("cost_basis", 0)) if holding.get("cost_basis") is not None else None,
                "gain_loss": str(holding.get("gain_loss", 0)) if holding.get("gain_loss") is not None else None,
        ...
        investment_accounts = [
            {
                "account_id": getattr(entry.get("account"), "id", None),
                "account_name": getattr(entry.get("account"), "account_name", ""),
                "institution_name": getattr(entry.get("account"), "institution_name", ""),
                "account_type": getattr(entry.get("account"), "account_type", ""),
                "value": str(entry.get("value", 0)),
                "holdings_count": entry.get("holdings_count", 0),
            }
            for entry in investment_data.get("account_summaries", [])
        ]
        ...
        recent_transactions = [
            {
                ...
                "security_ticker": transaction.security_ticker,
                "account_name": transaction.account.account_name if transaction.account else "",
            }
            for transaction in investment_data.get("recent_transactions", [])[:20]
        ]
        ...
        return JsonResponse(
            {
                ...
                "recent_transactions": recent_transactions,
                "gain_loss": str(investment_data.get("total_gain_loss", 0))
                if investment_data.get("total_gain_loss") is not None
                else None,
            }
        )
    except Exception as e:
        ...
        debt_accounts = [
            {
                ...
                "account_type": getattr(entry.get("account"), "account_type", ""),
                "institution_name": getattr(entry.get("account"), "institution_name", ""),
                "current_balance": str(getattr(entry.get("balance"), "current_balance", "0.00")),
                "interest_rate": str(getattr(entry.get("balance"), "interest_rate", ""))
                if getattr(entry.get("balance"), "interest_rate", None) is not None
                else None,
                "credit_limit": str(getattr(entry.get("balance"), "limit", ""))
                if getattr(entry.get("balance"), "limit", None) is not None
                else None,
                "debt_type": entry.get("debt_type"),
                "as_of": str(getattr(entry.get("balance"), "balance_date", "")),
            }
        ...
        debt_by_type = [
            {
                ...
                "balance": str(bucket.get("balance", 0)),
        ...
        return JsonResponse(
            {
                ...
                "total_credit_limit": str(debt_data.get("total_credit_limit", 0)),
                "credit_utilization": str(debt_data.get("credit_utilization"))
                if debt_data.get("credit_utilization") is not None
                else None,
